...
**代码链接：https://github.com/Sparky579/CS205-Project5**
...
3. 矩阵之间的赋值不用硬拷贝实现。在使用软拷贝的同时做好内存管理。
...
注意：我们再一次重载了=运算符，为了给矩阵赋值为一个数组。但是，用户需要让这个数组大小和矩阵大小匹配，否则难以检测数组的大小，会发生错误。
```
cout << "Case 3:\n";
...
```
#### Case 4: ROI测试
我们在这里用clone和ROIclone函数分别将整个矩阵和ROI区域硬拷贝出去，再进行比较。
这两个函数给用户提供了硬拷贝的接口。
代码：
```
mt21.setROI(0, 3, 1, 3);
...
#### 如何实现cout << mt1 + mt2这样的操作？
由于mt1 + mt2返回的是一个右值引用（临时量），正常的运算符重载不支持，这给用户的使用带来了一些困难。所以，我在这里采用了两种不同的重载，分别将左值和右值引用输出到流里。
